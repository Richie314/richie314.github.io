...
var bubble = {
    ...
    DisplayColumns() {
        ...
        for (var i = 0; i < bubble.buffer.length; i++) {
            ...
            bubble.ctx.fillRect(i * bubble.bar.wp, bubble.bar.hp * (bubble.buffer.length - bubble.buffer[i]), bubble.bar.wp, bubble.bar.hp * bubble.buffer[i])
        }
...
var selection = {
    ...
    DisplayColumns() {
        ...
        for (var i = 0; i < selection.buffer.length; i++) {
            ...
            selection.ctx.fillRect(i * selection.bar.wp, selection.bar.hp * (selection.buffer.length - selection.buffer[i]), selection.bar.wp, selection.bar.hp * selection.buffer[i])
        }
    ...
    DisplayColumnsText() {
        ...
        for (var i = 0; i < selection.buffer.length; i++) {
            ...
            selection.ctx.fillRect(i * selection.bar.wp, selection.bar.hp * (selection.buffer.length - selection.buffer[i]), selection.bar.wp, selection.bar.hp * selection.buffer[i])
            selection.ctx.fillText(selection.buffer[i], (i + 0.21) * selection.bar.wp, selection.bar.hp * (selection.buffer.length - selection.buffer[i] - 0.1))
        }
        selection.swaps.innerHTML = selection.swapsNum + '<br />'
    },
    Shuffle(size) {
    ...
    Sorting() {
        if (selection.i < selection.duration && selection.id !== 0) {
            if (selection.j < selection.buffer.length) {
                if (selection.buffer[selection.j] < selection.buffer[selection.index])
...
var quick = {
    ...
    check() {
        let order = true
        for (let i = 1; i< quick.buffer.length; i++)
            if (quick.buffer[i] < quick.buffer[i - 1])
                order = false
        if (order)
        {
            quick.pivot = -1
...
var cocktailshaker = {
    ...
    DisplayColumns() {
        ...
        for (var i = 0; i < cocktailshaker.buffer.length; i++) {
            ...
            cocktailshaker.ctx.fillRect(i * cocktailshaker.bar.wp, cocktailshaker.bar.hp * (cocktailshaker.buffer.length - cocktailshaker.buffer[i]), cocktailshaker.bar.wp, cocktailshaker.bar.hp * cocktailshaker.buffer[i])
        }
    ...
    DisplayColumnsText() {
        ...
        for (var i = 0; i < cocktailshaker.buffer.length; i++) {
            ...
            cocktailshaker.ctx.fillRect(i * cocktailshaker.bar.wp, cocktailshaker.bar.hp * (cocktailshaker.buffer.length - cocktailshaker.buffer[i]), cocktailshaker.bar.wp, cocktailshaker.bar.hp * cocktailshaker.buffer[i])
            cocktailshaker.ctx.fillText(cocktailshaker.buffer[i], (i + 0.21) * cocktailshaker.bar.wp, cocktailshaker.bar.hp * (cocktailshaker.buffer.length - cocktailshaker.buffer[i] - 0.1))
        }
        cocktailshaker.swaps.innerHTML = cocktailshaker.swapsNum + '<br>'
    },
    Shuffle(size) {
...
var insertion = {
    ...
    DisplayColumns() {
        ...
        for (var i = 0; i < insertion.buffer.length; i++) {
            ...
            insertion.ctx.fillRect(i * insertion.bar.wp, insertion.bar.hp * (insertion.buffer.length - insertion.buffer[i]), insertion.bar.wp, insertion.bar.hp * insertion.buffer[i])
        }
    ...
    DisplayColumnsText() {
        ...
        for (var i = 0; i < insertion.buffer.length; i++) {
            ...
            insertion.ctx.fillRect(i * insertion.bar.wp, insertion.bar.hp * (insertion.buffer.length - insertion.buffer[i]), insertion.bar.wp, insertion.bar.hp * insertion.buffer[i])
            insertion.ctx.fillText(insertion.buffer[i], (i + 0.21) * insertion.bar.wp, insertion.bar.hp * (insertion.buffer.length - insertion.buffer[i] - 0.1))
        }
        insertion.swaps.innerHTML = insertion.swapsNum + '<br />'
    },
    Shuffle(size) {
...
var oddeven = {
    ...
    DisplayColumns() {
        ...
        for (var i = 0; i < oddeven.buffer.length; i++) {
            ...
            oddeven.ctx.fillRect(i * oddeven.bar.wp, oddeven.bar.hp * (oddeven.buffer.length - oddeven.buffer[i]), oddeven.bar.wp, oddeven.bar.hp * oddeven.buffer[i])
        }
...
var merge = {
    ...
    check() {
        let order = true
        for (var i = 1; i< merge.buffer.length; i++)
            if (merge.buffer[i] < merge.buffer[i - 1])
                order = false
        if (order)
        {
            merge.m = -1
